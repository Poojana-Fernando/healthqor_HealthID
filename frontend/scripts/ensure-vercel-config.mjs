 ...
 * Set VERCEL_BACKEND_URL in Vercel project env (e.g. https://healthid-api.onrender.com).
 */
import fs from 'fs'
...
const raw = process.env.VERCEL_BACKEND_URL || process.env.RENDER_API_URL || ''
const backendBase = raw.replace(/\/$/, '')

if (!backendBase) {
  console.warn(
    '[ensure-vercel-config] VERCEL_BACKEND_URL is not set. ' +
